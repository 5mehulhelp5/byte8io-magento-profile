...
class ImportConfigData extends Command
{
    ...
    private function getProfileData(string $filename): array
    {
        ...
        $file = $directory->getAbsolutePath("plenty/$filename");
        if (!$directory->isReadable($file)) {
            throw new \Exception(sprintf('File "%s" is not readable.', $file));
        }

        $contents = $directory->readFile($file);
    ...
    private function saveProfileData(array $data): array
    {
        ...
        if ($saveRequestProfile) {
            $connection->insertOnDuplicate(
                $connection->getTableName(ProfileInterface::DB_TABLE_NAME),
                $saveRequestProfile
            );
        }
        ...
        if ($saveRequestConfig) {
            $connection->insertOnDuplicate(
                $connection->getTableName(ConfigInterface::DB_TABLE_NAME),
                $saveRequestConfig
            );
        }
