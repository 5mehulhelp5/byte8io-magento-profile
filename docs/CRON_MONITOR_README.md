...
### Admin Menu Path:
```
Byte8 → Profile Management → Cron Schedule Monitor
```

### Direct URL:
```
/admin/byte8/cronSchedule/index
```
...
**File**: `view/adminhtml/ui_component/byte8_cron_schedule_listing.xml`

```xml
<dataSource name="byte8_cron_schedule_listing_data_source"
            component="Byte8_Profile/js/grid/provider-auto-refresh">
...
```javascript
// Access the provider
var provider = registry.get('byte8_cron_schedule_listing.byte8_cron_schedule_listing_data_source');
...
2. **view/adminhtml/layout/byte8_cronschedule_index.xml** - Layout definition
3. **view/adminhtml/web/js/grid/provider-auto-refresh.js** - Auto-refresh provider
4. **etc/adminhtml/menu.xml** - Menu entry (updated)
5. **view/adminhtml/ui_component/byte8_cron_schedule_listing.xml** - Grid configuration (updated)

### ACL Resource:
```
Byte8_Profile::manage
```
